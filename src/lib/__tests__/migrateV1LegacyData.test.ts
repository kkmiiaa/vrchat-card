...
describe('migrateLegacyCardData（V1）', () => {
  describe('templateId !== v1', () => {
    it('無変換で返す', () => {
      ...
      expect(migrateLegacyCardData('v2', data)).not.toHaveProperty('vrchat')
    })
  })
  ...
  describe('冪等性（新フォーマット入力）', () => {
    it('sns も旧フォーマットキーもなければそのまま返す', () => {
      ...
      const result = migrateLegacyCardData('v1', data)
  ...
  describe('age 変換', () => {
    it('age.mode → age.searchTag に変換される', () => {
      const result = migrateLegacyCardData('v1', { age: { mode: '20s' } })
    ...
    it('age.searchTag が既にあれば age.mode を上書きしない', () => {
      const result = migrateLegacyCardData('v1', { age: { mode: '20s', searchTag: '30s' } })
      expect((result.age as Record<string, unknown>).searchTag).toBe('30s')
    })

    it('age がなければ何もしない', () => {
      const result = migrateLegacyCardData('v1', { name: 'test' })
