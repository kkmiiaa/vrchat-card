...
- [ ] **テンプレートごとの「作り方」エリア設定** — カード編集画面に表示される「自己紹介カードの作り方」（ステップ説明・外部リンク等）をテンプレートごとに `templates` テーブルで設定できるようにする。現在は旧メーカー（`/card/vrchat`）向けのハードコード内容が表示されている
- [ ] **テンプレートごとの X ハッシュタグ設定** — 「Xで投稿」時のハッシュタグをテンプレートごとに設定できるようにする（例: `#VRChat自己紹介カード`、`#アバター改変レシピ` 等）。現在は全テンプレート共通のハードコード値
