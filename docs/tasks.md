...
- [ ] **アバター改変レシピカード作成**（最優先）— テンプレートビルダーで作成
  - 3カラム固定グリッドレイアウト（左：プロフィール / 中央：ルーラー＋アバター画像 / 右：レシピ一覧）
  - アバター画像は透過PNG前提
  - ルーラーはSVGでシステム側が描画。身長数値を入力するとマーカー位置を自動計算
  - 必要な新規ブロック種別：①プロフィール行（アイコン＋ラベル＋値）②レシピアイテム（カテゴリ名＋サムネ＋商品名＋IDコード）③ベースアバター／ウェアカード ④ルーラー＋アバター合成エリア
  - 参考デザイン：モックアップ画像あり（2026-06-03 相談）
...
- [ ] **編集画面スマホプレビューが機能していない** — `/card/[cardId]/edit` のスマホ版プレビューが正常に表示されない
- [ ] **スマホ版フォントはみ出し** — スマホ表示時にフォントが大きくなりカード枠からはみ出す問題
