...
|---|---|---|---|
| 1 | `componentDef.defaultValue` | コンポーネント定義の初期値が正しいか | `''`（空文字） |
| 2 | `blockConfig.options=['A','B']` | 選択肢を設定したとき | 選択肢ボタンが描画される |
| 3 | 選択肢をクリック | ユーザーが選択したとき | 選択した値が `onChange` に渡される |
| 4 | `formLabel='信頼度'` | フォームラベルを設定したとき | 指定文字列がラベルとして表示される |
| 5 | card_data の型 | 保存される値の型が正しいか | `string` |
| 6 | CardItem に `'Trusted User'` を渡す | 選択済みの値を表示したとき | `'Trusted User'` が描画される |
| 7 | CardItem に `''` を渡す | 未選択の値を表示したとき | エラーなく描画される（空表示） |
...
| 2 | `blockConfig.options=['A','B','C']` | 選択肢を設定したとき | 選択肢が描画される |
| 3 | 選択肢を複数クリック | 複数選択したとき | 選択した値の配列が `onChange` に渡される |
| 4 | 選択済みの選択肢を再クリック | 選択を解除したとき | 値が除去された配列が渡される |
| 5 | `formLabel='プレイ環境'` | フォームラベルを設定したとき | 指定文字列がラベルとして表示される |
| 6 | card_data の型 | 保存される値の型が正しいか | `string[]` |
| 7 | CardItem に `['PCVR', 'Quest']` を渡す | 複数選択済みの値を表示したとき | 両方が描画される |
| 8 | CardItem に `[]` を渡す | 未選択の値を表示したとき | エラーなく描画される（空表示） |
...
| 2 | `blockConfig.marks=[...]` | チェック項目を設定したとき | 各項目が描画される |
| 3 | 項目を切り替え | OK/NG を変更したとき | 変更後の配列が `onChange` に渡される |
| 4 | `formLabel='インタラクション'` | フォームラベルを設定したとき | 指定文字列がラベルとして表示される |
| 5 | card_data の型 | 保存される値の型が正しいか | `{ key: string, value: boolean }[]` |
| 6 | CardItem に項目あり・一部 true の値を渡す | チェック済みの値を表示したとき | OK/NG の状態が描画される |
| 7 | CardItem に `[]` を渡す | 未入力の値を表示したとき | エラーなく描画される（空表示） |
...
| 2 | `blockConfig.items=[...]` | ステータス項目を設定したとき | 各項目が描画される |
| 3 | 項目をクリック | ステータスを選択したとき | 選択した項目が `onChange` に渡される |
| 4 | `formLabel='ステータス'` | フォームラベルを設定したとき | 指定文字列がラベルとして表示される |
| 5 | card_data の型 | 保存される値の型が正しいか | `Record<string, string>`（key: ステータスキー, value: 選択中の値） |
| 6 | CardItem に `{ blue: '募集中' }` を渡す | 選択済みの値を表示したとき | 対応する色とラベルが描画される |
| 7 | CardItem に `{}` を渡す | 未選択の値を表示したとき | エラーなく描画される（空表示） |
...
|---|---|---|---|
| 1 | `componentDef.defaultValue` | コンポーネント定義の初期値が正しいか | `{}`（空オブジェクト） |
| 2 | `blockConfig.platforms=['X','Discord']` | 複数プラットフォームを設定したとき | 各プラットフォームの入力欄が描画される |
| 3 | 各欄に入力 | それぞれのIDを入力したとき | `{ X: '入力値', Discord: '入力値' }` が渡される |
| 4 | `formLabel='SNS'` | フォームラベルを設定したとき | 指定文字列がラベルとして表示される |
| 5 | card_data の型 | 保存される値の型が正しいか | `Record<string, string>`（platform key → ID文字列） |
| 6 | CardItem に `{ X: '@foo', Discord: 'foo#1234' }` を渡す | 複数SNSを入力済みの値を表示したとき | 各プラットフォームのIDが描画される |
| 7 | CardItem に `{}` を渡す | 未入力の値を表示したとき | エラーなく描画される（空表示） |
