...
| 2 | 既存 CardV1/V2 をレンダラーで再現し互換性確認 | フェーズ1 |
...
| 現状 | 将来（案B） |
|---|---|
| `src/components/CardV1.tsx`（レイアウト固定） | 汎用レンダラー ＋ テンプレートJSON |
| テンプレート追加＝コードデプロイ | テンプレート追加＝DB操作のみ |
| `html-to-image` で画像生成 | 同一レンダラーで画像生成（互換維持） |
| 界隈ごとのカスタマイズ不可 | 界隈管理者がUIからテンプレートを作成・公開 |
