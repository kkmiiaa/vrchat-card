              ...
              <div className="flex flex-col gap-2">
                {siblingTemplates.filter(t => t.id !== templateId).map(t => (
                  <Link
                    key={t.id}
                    href={`/c/vrchat/${t.id}`}
                    className="group flex items-center justify-between px-4 py-3 bg-white rounded-xl border border-gray-100 hover:border-sky-200 hover:shadow-sm transition-all"
                  >
                    <div className="flex items-center gap-2.5">
                      ...
                      <span className="text-sm font-medium text-gray-700 group-hover:text-[#00AADB] transition-colors">{t.label}</span>
                    </div>
                    <svg className="w-4 h-4 text-gray-300 group-hover:text-sky-400 transition-colors" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                      <path strokeLinecap="round" strokeLinejoin="round" d="M9 5l7 7-7 7"/>
                    </svg>
                  </Link>
                ))}
