...
| 項目 | 現在 | 将来 |
|------|------|------|
| テンプレート定義の保存 | DB に保存済み（`templates` テーブル） | バージョニング対応（`template_versions` テーブルに分離） |
| テンプレートバージョン管理 | なし（上書き更新のみ） | Publish / Deprecate / Rollback 操作。詳細: `docs/template-versioning.md` |
| デザインパターン | なし | テンプレートバージョン内の見た目選択肢（テーマ・フォント・サーフェスの上書き）。詳細: `docs/template-versioning.md` |
| テンプレートビルダーの公開 | 管理者のみ（Admin UI） | ユーザーが自分のテンプレートを作成可能 |
| テーマ・背景 | テンプレート定義にハードコード or ユーザーがカード単位でカスタマイズ | デザインパターン経由で選択肢として管理 |
