...
- [ ] **アバター改変レシピカード作成**（最優先）— ブロック実装済み・DB登録が残り
  - 新規ブロック `item-list`（可変アイテムエントリ、Booth URL対応）実装済み
  - 新規ブロック `height-ruler`（SVGルーラー＋透過PNG重ね合わせ）実装済み
  - テンプレート定義 `avatar-recipe` / `avatar-recipe-simple` の TS ファイル作成済み
  - **残作業：admin テンプレートビルダーで DB に登録し、サンプルデータを設定する**
