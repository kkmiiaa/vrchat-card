...
| # | テスト内容 | 意味 | 期待値 |
|---|---|---|---|
| 1 | 初期値の型 | 初期状態の色数 | `['#60a5fa', '#4ade80', '#fbbf24', '#f87171']`（4色） |
| 2 | 色ピッカーの数 | 初期値の色数分UIが存在するか | カラーピッカーが 4 個描画される |
...
| # | テスト内容 | 意味 | 期待値 |
|---|---|---|---|
| 1 | 初期値の型 | 初期状態のデータ | 曜日・時間帯を含むオブジェクト |
...
| # | テスト内容 | 意味 | 期待値 |
|---|---|---|---|
| 1 | 初期値の型 | 未選択の状態 | `{ searchTag: '', display: '' }` |
| 2 | `global: true` | グローバルコンポーネントであるか | `componentDef.global === true` |
| 3 | 選択肢が固定 | blockConfig で年齢帯を変更できないか | 固定の年齢帯選択肢が表示される |
| 4 | 年齢帯を選択 | 年齢帯を選んだとき | `{ searchTag: '20s', display: '' }` 等が渡される |
| 5 | display 入力欄 | 自由入力欄が存在するか | input 要素が描画される |
