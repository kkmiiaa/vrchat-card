...
| 左 | 右 |
|---|---|
| vaacard（ロゴ） | ユーザーを探す（PCのみ） / HeaderAuth |

#### カード編集（`/card/[cardId]`）
| 左 | 右（PCのみ） | 右（共通） |
|---|---|---|
| vaacard / テンプレート名 / 下書き保存済み | Xでシェア / 画像で保存 / マイページに保存（未ログイン時は「マイページを作成」） | HeaderAuth |

- 下書き保存ステータスは `isLoggedIn && cardId` のときのみ表示
- ボタン群はモバイルでは FloatingButtons（画面右下）に移動

#### カード閲覧（`/card/[cardId]/view`）
| 左 | 右（PCのみ・オーナーのみ） | 右（共通） |
|---|---|---|
| vaacard（ロゴ） | 編集 / Xで共有 / 画像で保存 | HeaderAuth（白色テキスト） |
...
| 左 | 右 |
|---|---|
| vaacard（ロゴ） | ユーザーを探す（PCのみ） / HeaderAuth（`hideMyPage=true` のためオーナー表示時は非表示） |

- オーナー表示時: ロゴ右にマイページリンクは出ない（自分のページなので不要）
- プロフィール本文右上に「編集」「設定」ボタンを配置（ヘッダー外）

#### 探索（`/c/vrchat`）
| 左 | 右 |
|---|---|
| vaacard / VRChat | カードを作る / HeaderAuth |

#### テンプレート選択・利用規約・PP
| 左 | 右 |
|---|---|
| vaacard（ロゴ） | HeaderAuth |
