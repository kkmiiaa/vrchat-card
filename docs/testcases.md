...
| 1-2-5 | ログイン済み | ヘッダーに「マイページに保存」ボタンがある | 表示される |
...
| 1-3-2 | 未ログイン | ヘッダーに「編集」ボタンがない | 表示されない |
| 1-3-3 | 未ログイン | ヘッダーに「画像で保存」ボタンがない | 表示されない |
| 1-3-4 | 未ログイン | ヘッダーに「Xで共有」ボタンがない | 表示されない |
| 1-3-5 | 未ログイン | ヘッダーに「マイページに保存」ボタンがない | 表示されない |
| 1-3-6 | ログイン済み・非オーナー | ヘッダーに「編集」ボタンがない | 表示されない |
| 1-3-7 | ログイン済み・非オーナー | ヘッダーに「画像で保存」ボタンがない | 表示されない |
| 1-3-8 | ログイン済み・非オーナー | ヘッダーに「Xで共有」ボタンがない | 表示されない |
| 1-3-9 | ログイン済み・非オーナー | ヘッダーに「マイページに保存」ボタンがない | 表示されない |
| 1-3-10 | ログイン済み・オーナー | ヘッダーに「編集」ボタンがある | 表示される |
| 1-3-11 | ログイン済み・オーナー | ヘッダーに「画像で保存」ボタンがある | 表示される |
| 1-3-12 | ログイン済み・オーナー | ヘッダーに「Xで共有」ボタンがある | 表示される |
| 1-3-13 | ログイン済み・オーナー | ボタン順が「編集」→「画像で保存」→「Xで共有」の順 | 正しい順で表示 |
| 1-3-14 | ログイン済み・オーナー | ヘッダーに「マイページに保存」ボタンがない | 表示されない |
...
### TC-2-5: 公開フロー（visibility の遷移）

| # | 確認内容 | 期待結果 |
|---|---|---|
| 2-5-1 | 新規カードは初期状態で visibility=private | 探索ページに出ない |
| 2-5-2 | 編集画面の「マイページに保存」後 | visibility=public（未ログインユーザーがアクセスできる） |
| 2-5-3 | カード表示画面の「マイページに保存」後 | visibility=public |
| 2-5-4 | 公開後に再編集・debounce 保存しても | visibility=public のまま（下書きバッジが出ない） |
...
| # | 確認内容 | 期待結果 |
|---|---|---|
| 5-3-1 | 公開カードに `og:image` メタタグがある | 存在する |
| 5-3-2 | 公開カードの `og:image` がデフォルト画像でない | 実際のカード画像 URL が設定されている |
| 5-3-3 | 公開カードに `og:title` が設定されている | 空でない文字列 |
| 5-3-4 | 公開カードに `og:description` が設定されている | 空でない文字列 |
| 5-3-5 | 下書きカード（visibility=private）では `og:title` が返らない | メタタグなし |
| 5-3-6 | 下書きカードの `og:image` はデフォルト画像 | `/og-default.png` が設定される |
| 5-3-7 | `?v=N` 付き URL でも OGP が正しく返る | `og:image`・`og:title` が存在する |
...
| 6-A-4 | ログイン済み・V1カードなしで `/card/vrchat` にアクセス | `/card/new` にリダイレクト |
| 6-A-5 | ログイン済み・V1カードありで `/card/vrchat` にアクセス | 最古の V1 カードの編集画面（`/card/{id}/edit`）にリダイレクト |
...
### TC-6-E: 画像保存・Xシェア（未ログイン）

| # | 確認内容 | 期待結果 |
|---|---|---|
| 6-E-1 | 「画像で保存」を押しても 500 エラーが出ない | エラーなし |
| 6-E-2 | 「Xでシェア」を押すと Twitter/X の URL が開く（ログイン不要） | x.com または twitter.com に遷移、ログインリダイレクトなし |
| 6-E-3 | 「画像で保存」後にモーダルで「マイページに保存」の訴求が表示される | モーダルが表示される |
| 6-E-4 | 「Xへ共有」後にモーダルで「マイページに保存」の訴求が表示される | モーダルが表示される |
...
| 6-G-3 | 「マイページに保存」を押すと `/auth/login?next=/card/vrchat` に遷移 | `/auth/login` に遷移 |

### TC-6-H: 自動マイグレーション（未ログイン→アカウント作成フロー）

| # | 確認内容 | 期待結果 |
|---|---|---|
| 6-H-1 | 旧メーカーで入力後ログインするとデータが引き継がれる | localStorage のデータが CardEditor に反映される |
| 6-H-2 | マイグレーション後に 500 エラーが出ない | エラーなし |
| 6-H-3 | マイグレーション後に `/card/new` でテンプレ選択が再表示されない | `/card/{id}?created=1` に遷移する |

### TC-6-I: ログイン済みリダイレクト

| # | 確認内容 | 期待結果 |
|---|---|---|
| 6-I-1 | ログイン済み・V1カードなしで `/card/vrchat` にアクセス | `/card/new` にリダイレクト |
| 6-I-2 | ログイン済み・V1カード1枚ありで `/card/vrchat` にアクセス | そのカードの編集画面にリダイレクト |
| 6-I-3 | ログイン済み・V1カード複数ありで `/card/vrchat` にアクセス | 最古のカードの編集画面にリダイレクト |
...
| # | テスト内容 | 期待値 |
|---|---|---|
| 1 | ページにアクセスできる | 200・URL が `/upgrade` |
| 2 | タイトルが表示される | 「Proプランにアップグレード」が見える |
| 3 | Free プランの料金が表示される | ¥0 が見える |
| 4 | Pro プランの料金が表示される | ¥500/月 が見える |
| 5 | Pro プランの特徴が表示される | カード枚数無制限・フィルター解放が見える |
| 6 | アップグレードボタンが表示される | 「Proにアップグレードする」ボタンが見える |
| 7 | Stripe 表記が表示される | 「Stripe により安全に処理」が見える |
| 8 | 未ログインでボタン押下 | `/auth/login` へリダイレクト |
| 9 | 戻るボタンが表示される | 「戻る」ボタンが見える |

### TC-25-2: ログイン済み（Free プラン）

| # | テスト内容 | 期待値 |
|---|---|---|
| 1 | ページにアクセスできる | 見出しが表示される |
| 2 | アップグレードボタンが表示される | 「Proにアップグレードする」ボタンが見える |
| 3 | ボタン押下で checkout API が呼ばれる | `/api/stripe/checkout` POST リクエストが送信される |
| 4 | ボタン押下後に「処理中...」と表示される | ボタンがローディング状態になる |

### TC-25-3: API レベル確認

| # | テスト内容 | 期待値 |
|---|---|---|
| 1 | `/api/stripe/checkout` POST がログイン済みで 401 を返さない | 認証通過 |
| 2 | `/api/stripe/portal` POST がログイン済みで 401 を返さない | 認証通過 |
| 3 | Free ユーザーの portal リクエストは `no_stripe_customer` エラー | 400 + `{ error: 'no_stripe_customer' }` |
...
| # | テスト内容 | 期待値 |
|---|---|---|
| 1 | API が正常なレスポンス形式を返す | `cards[]`, `isPro`, `freeRemaining` が存在する |
| 2 | 公開カードが explore に含まれる | 作成した公開カードが返る |
| 3 | 非公開カードは explore に含まれない | 非公開カードが返らない |
| 4 | 各カードに profile フィールドが付与されている | `profile` キーが存在する（null 可） |

### TC-26-2: キーワード検索

| # | テスト内容 | 期待値 |
|---|---|---|
| 1 | カード名でヒットする | UNIQUE_TAG を含む名前で作成したカードが返る |
| 2 | 自己紹介でヒットする | selfIntro に UNIQUE_TAG を含むカードが返る |
| 3 | 存在しないキーワードでは 0 件 | cards.length === 0 |
| 4 | 検索後に freeRemaining がデクリメントされる | 3 → 2 |

### TC-26-3: 性別フィルター

| # | テスト内容 | 期待値 |
|---|---|---|
| 1 | gender=male で男性カードがヒットする | 男性カードが含まれる |
| 2 | gender=male で女性カードはヒットしない | 女性カードが含まれない |
| 3 | gender=female で女性カードがヒットする | 女性カードが含まれる |
| 4 | gender=female で男性カードはヒットしない | 男性カードが含まれない |

### TC-26-4: 言語フィルター

| # | テスト内容 | 期待値 |
|---|---|---|
| 1 | lang=日本語 で日本語カードがヒットする | 日本語カードが含まれる |
| 2 | lang=日本語 で English カードはヒットしない | English カードが含まれない |
| 3 | lang=English で English カードがヒットする | English カードが含まれる |

### TC-26-5: 年齢フィルター

| # | テスト内容 | 期待値 |
|---|---|---|
| 1 | age=18+ で 18+ カードがヒットする | 18+ カードが含まれる |
| 2 | age=18+ で 18歳未満カードはヒットしない | 18歳未満カードが含まれない |

### TC-26-6: 複合フィルター

| # | テスト内容 | 期待値 |
|---|---|---|
| 1 | gender=male&lang=日本語 で男性日本語カードがヒットする | 該当カードが含まれる |
| 2 | gender=male&lang=日本語 で女性英語カードはヒットしない | 非該当カードが含まれない |
| 3 | gender&lang&age の3条件で絞り込める | 全条件一致カードが含まれる |

### TC-26-7: Free プラン月3回制限

| # | テスト内容 | 期待値 |
|---|---|---|
| 1 | 1回目使用後: freeRemaining === 2 | 2 |
| 2 | 2回目使用後: freeRemaining === 1 | 1 |
| 3 | 3回目使用後: freeRemaining === 0 | 0 |
| 4 | 4回目: 403 limit_exceeded | `{ error: 'limit_exceeded', remaining: 0 }` |
| 5 | フィルターなしはカウントを消費しない | フィルターなし後の1回目は remaining=2 |
| 6 | リセット後はカウントが元に戻る | リセット後1回目は remaining=2 |
