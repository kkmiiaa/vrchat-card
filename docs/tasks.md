...
- ✅ **既存テンプレートのサンプルデータ設定** — `vrchat-simple` / `vrchat-glass` / `trpg-v1` の `sample_card_data` を管理画面から設定済み（2026-06-14）
...
- ✅ **Free プランでの検索・フィルター月3回解放** — `users` テーブルに `free_explore_count` / `free_explore_reset_month` カラム追加（migration 007）。API でカウント管理・超過時 403。UI で残り回数表示・超過時 UpgradeModal。未ログインはログイン誘導（2026-06-14）
- ✅ **検索・フィルター結果検証テスト** — `explore-search.spec.ts` を全面書き直し。基本動作・キーワード検索・性別/言語/年齢フィルター・複合フィルター・Free プラン月3回制限を網羅（TC-26、計29ケース）（2026-06-14）
...
- [ ] **デザイン修正** — 気になる箇所を随時修正
- ✅ **未完成カードの非公開化** — 一旦OK
- ✅ **テンプレートサンプルデータ入力** — 管理画面（TemplateBuilder）の「サンプルに設定」から設定済み（2026-06-14）
- ✅ **`/` スマホアニメーション** — 一旦OK
