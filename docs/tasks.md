...
- [ ] **`hasEmptyFields` の偽陽性** — `trustRank: ''`・`gender.display: ''` 等の非表示フィールドが常に空のため、通常の「マイページに保存」でも「未入力の項目があります」モーダルが表示される。非表示フィールドをスキップするか、ブロック定義の `required` フラグで判定する方式に変更が必要。
